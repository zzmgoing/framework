...
public final class ListPreloader {
    ...
    public ListPreloader(@NonNull RequestManager requestManager, @NonNull Callback callback, int maxPreload) {
        Util.require(maxPreload > 0, () -> "maxPreload must be greater than 0, but was " + maxPreload);

        this.requestManager = Util.requireNonNull(requestManager, "requestManager is null");
        this.callback = Util.requireNonNull(callback, "callback is null");
    ...
    public void attach(@NonNull Attacher attacher) {
        ...
        this.attacher = Util.requireNonNull(attacher, "attacher is null");
    ...
    public void onScrolled(int firstVisibleItem, int lastVisibleItem, int totalItemCount) {
        if (firstVisibleItem != -1 && lastVisibleItem != -1) {
            Util.require(firstVisibleItem >= 0 && firstVisibleItem < totalItemCount, () ->
                    "firstVisibleItem must be in range [0.." + totalItemCount + ") or -1, but was " + firstVisibleItem);
            Util.require(lastVisibleItem >= 0 && lastVisibleItem < totalItemCount, () ->
                    "lastVisibleItem must be in range [0.." + totalItemCount + ") or -1, but was " + lastVisibleItem);
            Util.require(firstVisibleItem <= lastVisibleItem, () ->
                    "firstVisibleItem (" + firstVisibleItem + ") must be less or equal to lastVisibleItem (" + lastVisibleItem + ")");
        }
        Util.require(totalItemCount >= 0, () -> "totalItemCount must be greater or equal to 0, but was " + totalItemCount);
    ...
    public void preload(@NonNull RequestBuilder<?> requestBuilder, int width, int height) {
        Util.require(width > 0, () -> "width must be greater than 0, but was " + width);
        Util.require(height > 0, () -> "height must be greater than 0, but was " + height);
